        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-dark-cardBg rounded-lg shadow-md p-6">
            ...
            {useMultipleMats && (
              <div className="mb-4">
                <label className="block text-sm font-medium text-light-textSecondary dark:text-dark-textSecondary mb-1">
                  Select Mat Position to Configure
                </label>
                <div className="flex space-x-2">
                  {[1, 2, 3].map(position => (
                    <button
                      key={position}
                      onClick={() => setActiveMatPosition(position)}
                      className={`px-3 py-1 text-xs rounded-md border ${
                        activeMatPosition === position
                          ? 'bg-primary text-white border-primary'
                          : 'bg-white dark:bg-dark-bg border-light-border dark:border-dark-border text-light-text dark:text-dark-text hover:border-primary'
                      }`}
                    >
                      {position === 1 ? 'Top' : position === 2 ? 'Middle' : 'Bottom'} Mat
                    </button>
                  ))}
                </div>
              </div>
